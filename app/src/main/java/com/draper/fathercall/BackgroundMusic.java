...
public class BackgroundMusic {
     ...
     * 根据path路径播放背景音乐
     *
     * @param resid  :assets中的音频路径
     * @param isLoop :是否循环播放
     */
    public void playBackgroundMusic(int resid, boolean isLoop) {
        if (mCurrentPath == 0) {
            // 这是第一次播放背景音乐--- it is the first time to play background music
            // 或者是执行end()方法后，重新被叫---or end() was called
            mBackgroundMediaPlayer = createMediaplayerFromAssets(resid);
            mCurrentPath = resid;
        } else {
            if (mCurrentPath != (resid)) {
     ...
     *
     * @param resid the path relative to assets
     * @return
     */
    private MediaPlayer createMediaplayerFromAssets(int resid) {
        MediaPlayer mediaPlayer = null;
        try {
            AssetFileDescriptor assetFileDescritor = mContext.getResources()
                    .openRawResourceFd(resid);
            // AssetFileDescriptor assetFileDescritor =
            // mContext.getAssets().openFd(path);
